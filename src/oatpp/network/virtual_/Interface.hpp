...
public:
  ...
  public:
     ...
     * Constructor.
     */
    ConnectionSubmission(bool valid)
      : m_valid(valid)
    {
    }

    /**
     * Set socket to be returned in call to &l:Interface::ConnectionSubmission::getSocket
     * ();/&l:Interface::ConnectionSubmission::getSocketNonBlocking ();.
...
public:
   ...
   * User may set waitingHandle = false and call &l:Interface::notifyAcceptors (); in order to break waiting loop. and
   * exit accept() method.
   * @return - `std::shared_ptr` to &id:oatpp::network::virtual_::Socket;.
   */
