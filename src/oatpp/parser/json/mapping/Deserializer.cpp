  ...
  : m_config(config)
{
  ...
  setDeserializerMethod(data::mapping::type::__class::AbstractVector::CLASS_ID,
                        &Deserializer::deserializeList<oatpp::AbstractVector>);
  setDeserializerMethod(data::mapping::type::__class::AbstractList::CLASS_ID,
                        &Deserializer::deserializeList<oatpp::AbstractList>);
  setDeserializerMethod(data::mapping::type::__class::AbstractUnorderedSet::CLASS_ID,
                        &Deserializer::deserializeList<oatpp::AbstractUnorderedSet>);

  setDeserializerMethod(data::mapping::type::__class::AbstractPairList::CLASS_ID,
                        &Deserializer::deserializeKeyValue<oatpp::AbstractFields>);
  setDeserializerMethod(data::mapping::type::__class::AbstractUnorderedMap::CLASS_ID,
                        &Deserializer::deserializeKeyValue<oatpp::AbstractUnorderedFields>);
}
...
void Deserializer::setDeserializerMethod(const data::mapping::type::ClassId& classId, DeserializerMethod method)
{
  ...
  if(id < m_methods.size()) {
    ...
    throw std::runtime_error(
     "[oatpp::parser::json::mapping::Deserializer::setDeserializerMethod()]: Error. Unknown classId");
  }
}
...
oatpp::Void Deserializer::deserializeBoolean(Deserializer* deserializer, parser::Caret& caret, const Type* const type)
{
  ...
  if(caret.isAtText("null", true)) {
    ...
    if(caret.isAtText("true", true)) {
      ...
      caret.setError(
       "[oatpp::parser::json::mapping::Deserializer::readBooleanValue()]: Error. 'true' or 'false' - expected.",
...
{
  {
    ...
    case 't':
      if(caret.isAtText("true"))
        return Boolean::Class::getType();
      break;
    case 'f':
      if(caret.isAtText("false"))
        return Boolean::Class::getType();
      break;
    default:
...
oatpp::Void Deserializer::deserializeEnum(Deserializer* deserializer, parser::Caret& caret, const Type* const type)
{
  ...
  auto polymorphicDispatcher =
   static_cast<const data::mapping::type::__class::AbstractEnum::AbstractPolymorphicDispatcher*>(
  ...
  case data::mapping::type::EnumInterpreterError::CONSTRAINT_NOT_NULL:
    caret.setError(
     "[oatpp::parser::json::mapping::Deserializer::deserializeEnum()]: Error. Enum constraint violated - 'NotNull'.");
    break;
  default:
  ...
  if(caret.canContinueAtChar('{', 1)) {
    ...
    while(!caret.isAtChar('}') && caret.canContinue()) {
      ...
      if(fieldIterator != fieldsMap.end()) {
        ...
        caret.skipBlankChars();
        if(!caret.canContinueAtChar(':', 1)) {
          caret.setError("[oatpp::parser::json::mapping::Deserializer::readObject()]: Error. ':' - expected",
                         ERROR_CODE_OBJECT_SCOPE_COLON_MISSING);
          return nullptr;
        }
        ...
        caret.skipBlankChars();
        if(!caret.canContinueAtChar(':', 1)) {
          caret.setError("[oatpp::parser::json::mapping::Deserializer::readObject()/if(config->allowUnknownFields){}]: "
                         "Error. ':' - expected",
                         ERROR_CODE_OBJECT_SCOPE_COLON_MISSING);
          return nullptr;
        }
        ...
        caret.setError("[oatpp::parser::json::mapping::Deserializer::readObject()]: Error. Unknown field",
                       ERROR_CODE_OBJECT_SCOPE_UNKNOWN_FIELD);
        return nullptr;
      }
...
oatpp::Void Deserializer::deserialize(parser::Caret& caret, const Type* const type)
{
  ...
  if(method) {
    ...
    throw std::runtime_error("[oatpp::parser::json::mapping::Deserializer::deserialize()]: "
                             "Error. No deserialize method for type '" +
                             std::string(type->classId.name) + "'");
  }
}
