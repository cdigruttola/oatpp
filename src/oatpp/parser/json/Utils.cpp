...
v_buff_size Utils::calcUnescapedStringSize(p_char8 data,
                                           v_buff_size size,
                                           v_int64& errorCode,
...
std::string Utils::unescapeStringToStdString(p_char8 data,
                                             v_buff_size size,
                                             v_int64& errorCode,
                                             v_buff_size& errorPosition)
{
...
p_char8 Utils::preparseString(ParsingCaret& caret, v_buff_size& size)
{
  ...
  if(caret.canContinueAtChar('"', 1)) {
    ...
    caret.setError("[oatpp::parser::json::Utils::preparseString()]: Error. '\"' - expected",
...
std::string Utils::parseStringToStdString(ParsingCaret& caret)
{
  ...
  if(data != nullptr) {
    ...
    if(errorCode != 0) {
      caret.setError(
       "[oatpp::parser::json::Utils::parseStringToStdString()]: Error. Call to unescapeStringToStdString() failed",
       errorCode);
      caret.setPosition(pos + errorPosition);
    } else {
      caret.setPosition(pos + size + 1);
