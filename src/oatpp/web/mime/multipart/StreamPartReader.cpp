...
void StreamPartReader::onNewPart(const std::shared_ptr<Part>& part)
{
  ...
  if(!m_streamProvider) {
    throw std::runtime_error(
     "[oatpp::web::mime::multipart::StreamPartReader::onNewPart()]: Error. Stream provider is nullptr.");
  }
  ...
  if(tag) {
    throw std::runtime_error(
     "[oatpp::web::mime::multipart::StreamPartReader::onNewPart()]: Error. "
...
void StreamPartReader::onPartData(const std::shared_ptr<Part>& part, p_char8 data, oatpp::v_io_size size)
{
  ...
  if(size > 0) {
    if(m_maxDataSize > 0 && tagObject->size + size > m_maxDataSize) {
      OATPP_LOGE("[oatpp::web::mime::multipart::StreamPartReader::onPartData()]",
                 "Error. Part size exceeds specified maxDataSize=%d",
                 m_maxDataSize);
      throw std::runtime_error(
       "[oatpp::web::mime::multipart::StreamPartReader::onPartData()]: Error. Part size exceeds specified maxDataSize");
    }
    auto res = tagObject->outputStream->writeExactSizeDataSimple(data, size);
    if(res != size) {
      OATPP_LOGE("[oatpp::web::mime::multipart::StreamPartReader::onPartData()]",
                 "Error. Failed to stream all data. Streamed %d/%d",
                 res,
                 size);
      throw std::runtime_error(
       "[oatpp::web::mime::multipart::StreamPartReader::onPartData()]: Error. Failed to stream all data.");
    }
    tagObject->size += res;
  } else {
    part->clearTag();
...
async::CoroutineStarter AsyncStreamPartReader::onNewPartAsync(const std::shared_ptr<Part>& part)
{
  ...
  public:
    ...
    Action act() override
    {
      ...
      if(!m_streamProvider) {
        throw std::runtime_error("[oatpp::web::mime::multipart::AsyncStreamPartReader::onNewPartAsync(){"
                                 "OnNewPartCoroutine}]: Error. Stream provider is nullptr.");
      }
      ...
      if(tag) {
        throw std::runtime_error(
         "[oatpp::web::mime::multipart::AsyncStreamPartReader::onNewPartAsync(){OnNewPartCoroutine}]: Error. "
         "Part tag object is not nullptr. Seems like this part is already being processed by another part reader.");
      }

      return m_streamProvider->getOutputStreamAsync(m_part, m_obtainedStream)
       .next(yieldTo(&OnNewPartCoroutine::onStreamObtained));
    }
...
async::CoroutineStarter AsyncStreamPartReader::onPartDataAsync(const std::shared_ptr<Part>& part,
                                                               p_char8 data,
                                                               oatpp::v_io_size size)
{
  ...
  if(size > 0) {
    if(m_maxDataSize > 0 && tagObject->size + size > m_maxDataSize) {
      OATPP_LOGE("[oatpp::web::mime::multipart::AsyncStreamPartReader::onPartDataAsync()]",
                 "Error. Part size exceeds specified maxDataSize=%d",
                 m_maxDataSize);
      throw std::runtime_error("[oatpp::web::mime::multipart::AsyncStreamPartReader::onPartDataAsync()]: Error. Part "
                               "size exceeds specified maxDataSize");
    }
...
async::CoroutineStarter AsyncStreamPartReader::onPartDone(const std::shared_ptr<Part>& part)
{
  ...
  public:
    ...
    Action act() override
    {
      return m_streamProvider->getInputStreamAsync(m_part, m_obtainedStream)
       .next(yieldTo(&OnPartDoneCoroutine::onStreamObtained));
    }
