...
void InMemoryPartReader::onNewPart(const std::shared_ptr<Part>& part)
{
  ...
  if(tag) {
    throw std::runtime_error(
     "[oatpp::web::mime::multipart::InMemoryPartReader::onNewPart()]: Error. "
...
void InMemoryPartReader::onPartData(const std::shared_ptr<Part>& part, p_char8 data, oatpp::v_io_size size)
{
  ...
  if(size > 0) {
    if(buffer->getSize() + size > m_maxDataSize) {
      OATPP_LOGE("[oatpp::web::mime::multipart::InMemoryPartReader::onPartData()]",
                 "Error. Part size exceeds specified maxDataSize=%d",
                 m_maxDataSize);
      throw std::runtime_error("[oatpp::web::mime::multipart::InMemoryPartReader::onPartData()]: Error. Part size "
                               "exceeds specified maxDataSize");
    }
    ...
    part->clearTag();
    auto stream =
     std::make_shared<data::stream::BufferInputStream>(fullData.getPtr(), fullData->getData(), fullData->getSize());
...
async::CoroutineStarter AsyncInMemoryPartReader::onNewPartAsync(const std::shared_ptr<Part>& part)
{
  ...
  if(tag) {
    throw std::runtime_error(
     "[oatpp::web::mime::multipart::AsyncInMemoryPartReader::onNewPartAsync()]: Error. "
...
async::CoroutineStarter AsyncInMemoryPartReader::onPartDataAsync(const std::shared_ptr<Part>& part,
                                                                 p_char8 data,
                                                                 oatpp::v_io_size size)
{
  ...
  if(size > 0) {
    if(buffer->getSize() + size > m_maxDataSize) {
      OATPP_LOGE("[oatpp::web::mime::multipart::AsyncInMemoryPartReader::onPartDataAsync()]",
                 "Error. Part size exceeds specified maxDataSize=%d",
                 m_maxDataSize);
      throw std::runtime_error("[oatpp::web::mime::multipart::AsyncInMemoryPartReader::onPartDataAsync()]: Error. Part "
