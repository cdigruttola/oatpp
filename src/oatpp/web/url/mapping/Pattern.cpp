...
std::shared_ptr<Pattern> Pattern::parse(p_char8 data, v_buff_size size)
{
  ...
  while(i < size) {
    ...
    if(a == '/') {
      ...
      if(i - lastPos > 0) {
        auto part =
         Part::createShared(Part::FUNCTION_CONST, oatpp::String((const char*)&data [ lastPos ], i - lastPos, true));
      ...
      if(size > lastPos) {
        auto part =
         Part::createShared(Part::FUNCTION_ANY_END, oatpp::String((const char*)&data [ lastPos ], size - lastPos, true));
        result->m_parts->pushBack(part);
      } else {
      ...
      if(i > lastPos) {
        auto part =
         Part::createShared(Part::FUNCTION_VAR, oatpp::String((const char*)&data [ lastPos ], i - lastPos, true));
        result->m_parts->pushBack(part);
      } else {
    ...
    if(part->function == Part::FUNCTION_CONST) {
      ...
      if(caret.canContinue() && !caret.isAtChar('/')) {
        if(caret.isAtChar('?') && (curr == nullptr || curr->getData()->function == Part::FUNCTION_ANY_END)) {
          matchMap.m_tail =
           StringKeyLabel(url.getMemoryHandle(), caret.getCurrData(), caret.getDataSize() - caret.getPosition());
          return true;
        }
        return false;
      }
