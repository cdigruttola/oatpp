...
std::shared_ptr<HttpRequestExecutor> HttpRequestExecutor::createShared(
...
std::shared_ptr<HttpRequestExecutor::ConnectionHandle> HttpRequestExecutor::getConnection()
{
  ...
  if(!connection) {
    throw RequestExecutionError(
     RequestExecutionError::ERROR_CODE_CANT_CONNECT,
 ...
 HttpRequestExecutor::getConnectionAsync()
{
  ...
  class GetConnectionCoroutine
    : public oatpp::async::CoroutineWithResult<GetConnectionCoroutine, const std::shared_ptr<ConnectionHandle>&> {
  private:
...
std::shared_ptr<HttpRequestExecutor::Response> HttpRequestExecutor::executeOnce(
 const String& method,
 const String& path,
 ...
 const std::shared_ptr<ConnectionHandle>& connectionHandle)
{
  ...
  request->putHeaderIfNotExists(oatpp::web::protocol::http::Header::CONNECTION,
                                oatpp::web::protocol::http::Header::Value::CONNECTION_KEEP_ALIVE);

  oatpp::data::share::MemoryLabel buffer(
   oatpp::base::StrBuffer::createShared(oatpp::data::buffer::IOBuffer::BUFFER_SIZE));
  ...
  if(error.status.code != 0) {
    ...
    throw RequestExecutionError(
     RequestExecutionError::ERROR_CODE_CANT_PARSE_STARTING_LINE,
  ...
  auto bodyStream =
   oatpp::data::stream::InputStreamBufferedProxy::createShared(connection,
                                                               buffer,
                                       ...
                                       const std::shared_ptr<ConnectionHandle>& connectionHandle)
{
  ...
  class ExecutorCoroutine
    : public oatpp::async::CoroutineWithResult<ExecutorCoroutine,
                                               const std::shared_ptr<HttpRequestExecutor::Response>&> {
  private:
  ...
  public:
    ...
    Action act() override
    {
      ...
      return OutgoingRequest::sendAsync(request, m_upstream)
       .next(m_upstream->flushAsync())
       .next(yieldTo(&ExecutorCoroutine::readResponse));
    }
    ...
    Action onHeadersParsed(const ResponseHeadersReader::Result& result)
    {
      ...
      auto bodyStream =
       oatpp::data::stream::InputStreamBufferedProxy::createShared(m_connection,
                                                                   m_buffer,
                                                                   result.bufferPosStart,
                                                                   result.bufferPosEnd,
                                                                   result.bufferPosStart != result.bufferPosEnd);
