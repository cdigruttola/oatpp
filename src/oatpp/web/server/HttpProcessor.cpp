...
HttpProcessor::Components::Components(
 const std::shared_ptr<HttpRouter>& pRouter,
...
HttpProcessor::ProcessingResources::ProcessingResources(
 const std::shared_ptr<Components>& pComponents, const std::shared_ptr<oatpp::data::stream::IOStream>& pConnection)
  ...
  , inStream(data::stream::InputStreamBufferedProxy::createShared(
     connection, base::StrBuffer::createShared(data::buffer::IOBuffer::BUFFER_SIZE)))
{
}
...
bool HttpProcessor::processNextRequest(ProcessingResources& resources)
{
  ...
  auto route =
   resources.components->router->getRoute(headersReadResult.startingLine.method, headersReadResult.startingLine.path);

  if(!route) {
    auto response =
     resources.components->errorHandler->handleError(protocol::http::Status::CODE_404, "Current url has no mapping");
    response->send(resources.connection.get(), &resources.headersOutBuffer, nullptr);
    return false;
  }
  ...
  case protocol::http::utils::CommunicationUtils::CONNECTION_STATE_KEEP_ALIVE:
    return true;
  ...
  case protocol::http::utils::CommunicationUtils::CONNECTION_STATE_UPGRADE: {
    ...
    if(handler) {
      ...
      OATPP_LOGW("[oatpp::web::server::HttpProcessor::processNextRequest()]",
                 "Warning. ConnectionUpgradeHandler not set!");
    }
...
HttpProcessor::Coroutine::Coroutine(const std::shared_ptr<Components>& components,
  ...
  , m_headersReader(
     &m_headersInBuffer, components->config->headersReaderChunkSize, components->config->headersReaderMaxSize)
  , m_headersOutBuffer(std::make_shared<oatpp::data::stream::BufferOutputStream>(
     components->config->headersOutBufferInitial, components->config->headersOutBufferGrow))
  , m_inStream(data::stream::InputStreamBufferedProxy::createShared(
     m_connection, base::StrBuffer::createShared(data::buffer::IOBuffer::BUFFER_SIZE)))
...
oatpp::async::Action HttpProcessor::Coroutine::onHeadersParsed(const RequestHeadersReader::Result& headersReadResult)
{
  ...
  m_currentRoute = m_components->router->getRoute(headersReadResult.startingLine.method.toString(),
                                                  headersReadResult.startingLine.path.toString());

  if(!m_currentRoute) {
    m_currentResponse =
     m_components->errorHandler->handleError(protocol::http::Status::CODE_404, "Current url has no mapping");
...
HttpProcessor::Coroutine::Action HttpProcessor::Coroutine::onResponseFormed()
{
  ...
  m_connectionState =
   oatpp::web::protocol::http::utils::CommunicationUtils::considerConnectionState(m_currentRequest, m_currentResponse);

  auto contentEncoderProvider =
   protocol::http::utils::CommunicationUtils::selectEncoder(m_currentRequest, m_components->contentEncodingProviders);

  return protocol::http::outgoing::Response::sendAsync(m_currentResponse,
                                                       m_connection,
                                                       m_headersOutBuffer,
                                                       contentEncoderProvider)
...
HttpProcessor::Coroutine::Action HttpProcessor::Coroutine::onRequestDone()
{
  ...
  if(m_connectionState == oatpp::web::protocol::http::utils::CommunicationUtils::CONNECTION_STATE_UPGRADE) {
    ...
    if(handler) {
      ...
      OATPP_LOGW("[oatpp::web::server::HttpProcessor::Coroutine::onRequestDone()]",
                 "Warning. ConnectionUpgradeHandler not set!");
    }
  }
...
HttpProcessor::Coroutine::Action HttpProcessor::Coroutine::handleError(Error* error)
{
  ...
  if(error) {
    ...
    if(m_currentResponse) {
      // OATPP_LOGE("[oatpp::web::server::HttpProcessor::Coroutine::handleError()]", "Unhandled error. '%s'. Dropping
