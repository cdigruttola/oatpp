...
v_io_size ResponseHeadersReader::readHeadersSectionIterative(
...
ResponseHeadersReader::Result ResponseHeadersReader::readHeaders(
 const std::shared_ptr<oatpp::data::stream::IOStream>& connection, http::HttpError::Info& error)
{
  ...
  while(!iteration.done) {
    ...
    if(!action.isNone()) {
      throw std::runtime_error(
       "[oatpp::web::protocol::http::incoming::ResponseHeadersReader::readHeaders]: Error. Async action is unexpected.");
    }
...
oatpp::async::CoroutineStarterForResult<const ResponseHeadersReader::Result&> ResponseHeadersReader::readHeadersAsync(
 const std::shared_ptr<oatpp::data::stream::IOStream>& connection)
{
  ...
  public:
    ReaderCoroutine(ResponseHeadersReader* _this, const std::shared_ptr<oatpp::data::stream::IOStream>& connection)
      : m_this(_this)
    ...
    Action parseHeaders()
    {
      ...
      if(status.code == 0) {
        ...
        if(status.code == 0) {
          ...
          return error<Error>("[oatpp::web::protocol::http::incoming::ResponseHeadersReader::readHeadersAsync()]: "
                              "Error. Error occurred while parsing headers.");
        }
      } else {
        return error<Error>("[oatpp::web::protocol::http::incoming::ResponseHeadersReader::readHeadersAsync()]: Error. "
