...
public:
  Range(const oatpp::String& pUnits, v_int64 pStart, v_int64 pEnd)
    : units(pUnits)
    , start(pStart)
    , end(pEnd)
  {
...
public:
  ContentRange(const oatpp::String& pUnits, v_int64 pStart, v_int64 pEnd, v_int64 pSize, bool pIsSizeKnown)
    : units(pUnits)
    , start(pStart)
    , end(pEnd)
    , size(pSize)
...
class Parser {
private:
  static oatpp::data::share::StringKeyLabelCI_FAST parseHeaderNameLabel(
...
public:
   ...
   */
  static void parseHeaderValueData(HeaderValueData& data,
