...
#define REQUEST(TYPE, ...)                                                  \
  OATPP_MACRO_API_CONTROLLER_PARAM(OATPP_MACRO_API_CONTROLLER_REQUEST,      \
                                   OATPP_MACRO_API_CONTROLLER_REQUEST_INFO, \
                                   TYPE,                                    \
                                   (__VA_ARGS__))

#define HEADER(TYPE, ...)                                                  \
  OATPP_MACRO_API_CONTROLLER_PARAM(OATPP_MACRO_API_CONTROLLER_HEADER,      \
                                   OATPP_MACRO_API_CONTROLLER_HEADER_INFO, \
                                   TYPE,                                   \
                                   (__VA_ARGS__))

#define PATH(TYPE, ...)                                                  \
  OATPP_MACRO_API_CONTROLLER_PARAM(OATPP_MACRO_API_CONTROLLER_PATH,      \
                                   OATPP_MACRO_API_CONTROLLER_PATH_INFO, \
                                   TYPE,                                 \
                                   (__VA_ARGS__))

#define QUERIES(TYPE, ...)                                                  \
  OATPP_MACRO_API_CONTROLLER_PARAM(OATPP_MACRO_API_CONTROLLER_QUERIES,      \
                                   OATPP_MACRO_API_CONTROLLER_QUERIES_INFO, \
                                   TYPE,                                    \
                                   (__VA_ARGS__))

#define QUERY(TYPE, ...)                                                  \
  OATPP_MACRO_API_CONTROLLER_PARAM(OATPP_MACRO_API_CONTROLLER_QUERY,      \
                                   OATPP_MACRO_API_CONTROLLER_QUERY_INFO, \
                                   TYPE,                                  \
                                   (__VA_ARGS__))

#define BODY_STRING(TYPE, ...)                                                  \
  OATPP_MACRO_API_CONTROLLER_PARAM(OATPP_MACRO_API_CONTROLLER_BODY_STRING,      \
                                   OATPP_MACRO_API_CONTROLLER_BODY_STRING_INFO, \
                                   TYPE,                                        \
                                   (__VA_ARGS__))

#define BODY_DTO(TYPE, ...)                                                  \
  OATPP_MACRO_API_CONTROLLER_PARAM(OATPP_MACRO_API_CONTROLLER_BODY_DTO,      \
                                   OATPP_MACRO_API_CONTROLLER_BODY_DTO_INFO, \
                                   TYPE,                                     \
                                   (__VA_ARGS__))
...
#define OATPP_MACRO_API_CONTROLLER_REQUEST(TYPE, PARAM_LIST) const auto& OATPP_MACRO_FIRSTARG PARAM_LIST = __request;
...
#define OATPP_MACRO_API_CONTROLLER_HEADER_1(TYPE, NAME)                                                                 \
  ...
  const auto& NAME =                                                                                                    \
   ApiController::TypeInterpretation<TYPE>::fromString(#TYPE, __param_str_val_##NAME, __param_validation_check_##NAME); \
  if(!__param_validation_check_##NAME) {                                                                                \
    return ApiController::handleError(Status::CODE_400,                                                                 \
                                      "Invalid HEADER parameter '" #NAME "'. Expected type is '" #TYPE "'");            \
  }
...
#define OATPP_MACRO_API_CONTROLLER_HEADER_2(TYPE, NAME, QUALIFIER)                                                      \
  ...
  if(!__param_str_val_##NAME) {                                                                                         \
    return ApiController::handleError(Status::CODE_400, oatpp::String("Missing HEADER parameter '") + QUALIFIER + "'"); \
  }                                                                                                                     \
  bool __param_validation_check_##NAME;                                                                                 \
  const auto& NAME =                                                                                                    \
   ApiController::TypeInterpretation<TYPE>::fromString(#TYPE, __param_str_val_##NAME, __param_validation_check_##NAME); \
  if(!__param_validation_check_##NAME) {                                                                                \
    return ApiController::handleError(Status::CODE_400,                                                                 \
                                      oatpp::String("Invalid HEADER parameter '") + QUALIFIER +                         \
                                       "'. Expected type is '" #TYPE "'");                                              \
  }

#define OATPP_MACRO_API_CONTROLLER_HEADER(TYPE, PARAM_LIST)                     \
  OATPP_MACRO_API_CONTROLLER_MACRO_SELECTOR(OATPP_MACRO_API_CONTROLLER_HEADER_, \
                                            TYPE,                               \
                                            OATPP_MACRO_UNFOLD_VA_ARGS PARAM_LIST)

// __INFO

#define OATPP_MACRO_API_CONTROLLER_HEADER_INFO_1(TYPE, NAME) info->headers.add(#NAME, TYPE::Class::getType());
...
#define OATPP_MACRO_API_CONTROLLER_HEADER_INFO(TYPE, PARAM_LIST)                     \
  OATPP_MACRO_API_CONTROLLER_MACRO_SELECTOR(OATPP_MACRO_API_CONTROLLER_HEADER_INFO_, \
                                            TYPE,                                    \
                                            OATPP_MACRO_UNFOLD_VA_ARGS PARAM_LIST)
...
#define OATPP_MACRO_API_CONTROLLER_PATH_1(TYPE, NAME)                                                                   \
  ...
  if(!__param_validation_check_##NAME) {                                                                                \
    return ApiController::handleError(Status::CODE_400,                                                                 \
                                      "Invalid PATH parameter '" #NAME "'. Expected type is '" #TYPE "'");              \
  }
...
#define OATPP_MACRO_API_CONTROLLER_PATH_2(TYPE, NAME, QUALIFIER)                                                        \
  ...
  if(!__param_str_val_##NAME) {                                                                                         \
    return ApiController::handleError(Status::CODE_400, oatpp::String("Missing PATH parameter '") + QUALIFIER + "'");   \
  }                                                                                                                     \
  bool __param_validation_check_##NAME;                                                                                 \
  const auto NAME =                                                                                                     \
   ApiController::TypeInterpretation<TYPE>::fromString(#TYPE, __param_str_val_##NAME, __param_validation_check_##NAME); \
  if(!__param_validation_check_##NAME) {                                                                                \
    return ApiController::handleError(Status::CODE_400,                                                                 \
                                      oatpp::String("Invalid PATH parameter '") + QUALIFIER +                           \
                                       "'. Expected type is '" #TYPE "'");                                              \
  }

#define OATPP_MACRO_API_CONTROLLER_PATH(TYPE, PARAM_LIST)                     \
  OATPP_MACRO_API_CONTROLLER_MACRO_SELECTOR(OATPP_MACRO_API_CONTROLLER_PATH_, \
                                            TYPE,                             \
                                            OATPP_MACRO_UNFOLD_VA_ARGS PARAM_LIST)

// __INFO

#define OATPP_MACRO_API_CONTROLLER_PATH_INFO_1(TYPE, NAME) info->pathParams.add(#NAME, TYPE::Class::getType());
...
#define OATPP_MACRO_API_CONTROLLER_PATH_INFO(TYPE, PARAM_LIST)                     \
  OATPP_MACRO_API_CONTROLLER_MACRO_SELECTOR(OATPP_MACRO_API_CONTROLLER_PATH_INFO_, \
                                            TYPE,                                  \
                                            OATPP_MACRO_UNFOLD_VA_ARGS PARAM_LIST)
...
#define OATPP_MACRO_API_CONTROLLER_QUERY_1(TYPE, NAME)                                                                  \
  ...
  if(!__param_validation_check_##NAME) {                                                                                \
    return ApiController::handleError(Status::CODE_400,                                                                 \
                                      "Invalid QUERY parameter '" #NAME "'. Expected type is '" #TYPE "'");             \
  }
...
#define OATPP_MACRO_API_CONTROLLER_QUERY_2(TYPE, NAME, QUALIFIER)                                                       \
  ...
  if(!__param_str_val_##NAME) {                                                                                         \
    return ApiController::handleError(Status::CODE_400, oatpp::String("Missing QUERY parameter '") + QUALIFIER + "'");  \
  }                                                                                                                     \
  bool __param_validation_check_##NAME;                                                                                 \
  const auto& NAME =                                                                                                    \
   ApiController::TypeInterpretation<TYPE>::fromString(#TYPE, __param_str_val_##NAME, __param_validation_check_##NAME); \
  if(!__param_validation_check_##NAME) {                                                                                \
    return ApiController::handleError(Status::CODE_400,                                                                 \
                                      oatpp::String("Invalid QUERY parameter '") + QUALIFIER +                          \
                                       "'. Expected type is '" #TYPE "'");                                              \
  }

#define OATPP_MACRO_API_CONTROLLER_QUERY(TYPE, PARAM_LIST)                     \
  OATPP_MACRO_API_CONTROLLER_MACRO_SELECTOR(OATPP_MACRO_API_CONTROLLER_QUERY_, \
                                            TYPE,                              \
                                            OATPP_MACRO_UNFOLD_VA_ARGS PARAM_LIST)

// __INFO

#define OATPP_MACRO_API_CONTROLLER_QUERY_INFO_1(TYPE, NAME) info->queryParams.add(#NAME, TYPE::Class::getType());
...
#define OATPP_MACRO_API_CONTROLLER_QUERY_INFO(TYPE, PARAM_LIST)                     \
  OATPP_MACRO_API_CONTROLLER_MACRO_SELECTOR(OATPP_MACRO_API_CONTROLLER_QUERY_INFO_, \
                                            TYPE,                                   \
                                            OATPP_MACRO_UNFOLD_VA_ARGS PARAM_LIST)
...
#define OATPP_MACRO_API_CONTROLLER_BODY_DTO(TYPE, PARAM_LIST)                                                    \
  const auto& OATPP_MACRO_FIRSTARG PARAM_LIST = __request->readBodyToDto<TYPE>(getDefaultObjectMapper().get());  \
  if(!OATPP_MACRO_FIRSTARG PARAM_LIST) {                                                                         \
    return ApiController::handleError(Status::CODE_400,                                                          \
                                      "Missing valid body parameter '" OATPP_MACRO_FIRSTARG_STR PARAM_LIST "'"); \
  }
...
#define OATPP_MACRO_API_CONTROLLER_FOR_EACH_PARAM_PUT(INDEX, COUNT, X) OATPP_MACRO_API_CONTROLLER_PARAM_MACRO X

#define OATPP_MACRO_API_CONTROLLER_FOR_EACH_PARAM_CALL_FIRST(INDEX, COUNT, X) OATPP_MACRO_API_CONTROLLER_PARAM_NAME X

#define OATPP_MACRO_API_CONTROLLER_FOR_EACH_PARAM_CALL_REST(INDEX, COUNT, X) , OATPP_MACRO_API_CONTROLLER_PARAM_NAME X

#define OATPP_MACRO_API_CONTROLLER_FOR_EACH_PARAM_INFO(INDEX, COUNT, X) OATPP_MACRO_API_CONTROLLER_PARAM_INFO X
...
#define ENDPOINT_INFO(NAME)                                                   \
  ...
  {                                                                           \
    ...
    return info;                                                              \
                                                                              ...
                                                                              \
  const std::shared_ptr<Endpoint::Info> Z__ENDPOINT_ADDITIONAL_INFO_##NAME =  \
   Z__ENDPOINT_CREATE_ADDITIONAL_INFO_##NAME();                               \
                                                                              \
...
#define OATPP_MACRO_API_CONTROLLER_ENDPOINT_DECL_0(NAME, METHOD, PATH)         \
  ...
  EndpointInfoBuilder Z__CREATE_ENDPOINT_INFO_##NAME = [ this ]() {            \
    ...
    return info;                                                               \
                                                                               ...
                                                                               \
  const std::shared_ptr<Endpoint> Z__ENDPOINT_##NAME =                         \
   createEndpoint(m_endpoints, Z__ENDPOINT_HANDLER_GET_INSTANCE_##NAME(this), Z__CREATE_ENDPOINT_INFO_##NAME);
...
#define OATPP_MACRO_API_CONTROLLER_ENDPOINT_0(NAME, METHOD, PATH)                         \
  ...
  OATPP_MACRO_API_CONTROLLER_ENDPOINT_DECL_0(NAME, METHOD, PATH)                          \
                                                                                          \
  std::shared_ptr<oatpp::web::protocol::http::outgoing::Response> Z__PROXY_METHOD_##NAME( \
...
#define OATPP_MACRO_API_CONTROLLER_ENDPOINT_1(NAME, METHOD, PATH, ...)                                   \
  ...
  {                                                                                                      \
    ...
    return NAME(OATPP_MACRO_FOREACH_FIRST_AND_REST(OATPP_MACRO_API_CONTROLLER_FOR_EACH_PARAM_CALL_FIRST, \
                                                   OATPP_MACRO_API_CONTROLLER_FOR_EACH_PARAM_CALL_REST,  \
                                                   __VA_ARGS__));                                        \
  }                                                                                                      \
                                                                                                         \
  std::shared_ptr<oatpp::web::protocol::http::outgoing::Response> NAME(                                  \
   OATPP_MACRO_FOREACH_FIRST_AND_REST(OATPP_MACRO_API_CONTROLLER_FOR_EACH_PARAM_DECL_FIRST,              \
...
#define ENDPOINT(METHOD, PATH, ...)                                                                \
  OATPP_MACRO_EXPAND(OATPP_MACRO_MACRO_BINARY_SELECTOR(OATPP_MACRO_API_CONTROLLER_ENDPOINT_MACRO_, \
                                                       (__VA_ARGS__))(METHOD, PATH, __VA_ARGS__))
...
#define ENDPOINT_INTERCEPTOR(ENDPOINT_NAME, NAME)                                                                       \
                                                                                                                        \
  Handler<oatpp::web::server::api::ApiController>::Method Z__INTERCEPTOR_METHOD_##ENDPOINT_NAME##_##NAME =              \
   Z__INTERCEPTOR_METHOD_SET_##ENDPOINT_NAME##_##NAME(this);                                                            \
                                                                                                                        \
  template<class T>                                                                                                     \
  Handler<oatpp::web::server::api::ApiController>::Method Z__INTERCEPTOR_METHOD_SET_##ENDPOINT_NAME##_##NAME(           \
   T* controller)                                                                                                       \
  {                                                                                                                     \
    return static_cast<Handler<oatpp::web::server::api::ApiController>::Method>(                                        \
     Z__ENDPOINT_HANDLER_GET_INSTANCE_##ENDPOINT_NAME(controller)                                                       \
      ->setMethod(&T::Z__PROXY_INTERCEPTOR_METHOD_##ENDPOINT_NAME##_##NAME));                                           \
  }                                                                                                                     \
                                                                                                                        \
  std::shared_ptr<oatpp::web::protocol::http::outgoing::Response> Z__PROXY_INTERCEPTOR_METHOD_##ENDPOINT_NAME##_##NAME( \
  ...
  {                                                                                                                     \
    ...
    return Z__USER_INTERCEPTOR_METHOD_##ENDPOINT_NAME##_##NAME<T>(intercepted, request);                                \
                                                                                                                        ...
                                                                                                                        \
  template<class T>                                                                                                     \
  std::shared_ptr<oatpp::web::protocol::http::outgoing::Response> Z__USER_INTERCEPTOR_METHOD_##ENDPOINT_NAME##_##NAME(  \
...
public:                                                                                 \
  NAME(__ControllerType* pController, const std::shared_ptr<IncomingRequest>& pRequest) \
...
#define ENDPOINT_INTERCEPTOR_ASYNC(ENDPOINT_NAME, NAME)                                                               \
                                                                                                                      \
  Handler<oatpp::web::server::api::ApiController>::MethodAsync Z__INTERCEPTOR_METHOD_##ENDPOINT_NAME##_##NAME =       \
   Z__INTERCEPTOR_METHOD_SET_##ENDPOINT_NAME##_##NAME(this);                                                          \
                                                                                                                      \
  template<class T>                                                                                                   \
  Handler<oatpp::web::server::api::ApiController>::MethodAsync Z__INTERCEPTOR_METHOD_SET_##ENDPOINT_NAME##_##NAME(    \
   T* controller)                                                                                                     \
  {                                                                                                                   \
    return static_cast<Handler<oatpp::web::server::api::ApiController>::MethodAsync>(                                 \
     Z__ENDPOINT_HANDLER_GET_INSTANCE_##ENDPOINT_NAME(controller)                                                     \
      ->setMethodAsync(&T::Z__PROXY_INTERCEPTOR_METHOD_##ENDPOINT_NAME##_##NAME));                                    \
  }                                                                                                                   \
                                                                                                                      \
  oatpp::async::CoroutineStarterForResult<const std::shared_ptr<oatpp::web::protocol::http::outgoing::Response>&>     \
   Z__PROXY_INTERCEPTOR_METHOD_##ENDPOINT_NAME##_##NAME(                                                              \
    const std::shared_ptr<oatpp::web::protocol::http::incoming::Request>& request)                                    \
  {                                                                                                                   \
    return Z__USER_PROXY_INTERCEPTOR_METHOD_##ENDPOINT_NAME##_##NAME(this, request);                                  \
  }                                                                                                                   \
