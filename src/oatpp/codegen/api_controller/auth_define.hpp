...
#define AUTHORIZATION(TYPE, ...)                                                  \
  OATPP_MACRO_API_CONTROLLER_PARAM(OATPP_MACRO_API_CONTROLLER_AUTHORIZATION,      \
                                   OATPP_MACRO_API_CONTROLLER_AUTHORIZATION_INFO, \
                                   TYPE,                                          \
                                   (__VA_ARGS__))
...
#define OATPP_MACRO_API_CONTROLLER_AUTHORIZATION_1(TYPE, NAME)                                           \
  ...
  std::shared_ptr<oatpp::web::server::handler::AuthorizationObject> __param_aosp_val_##NAME =            \
   ApiController::handleDefaultAuthorization(__param_str_val_##NAME);                                    \
...
#define OATPP_MACRO_API_CONTROLLER_AUTHORIZATION_2(TYPE, NAME, AUTH_HANDLER)                               \
  ...
  std::shared_ptr<oatpp::web::server::handler::AuthorizationObject> __param_aosp_val_##NAME =              \
   __auth_handler_##NAME->handleAuthorization(__param_str_val_##NAME);                                     \
  TYPE NAME = std::static_pointer_cast<TYPE::element_type>(__param_aosp_val_##NAME);

#define OATPP_MACRO_API_CONTROLLER_AUTHORIZATION(TYPE, PARAM_LIST)                     \
  OATPP_MACRO_API_CONTROLLER_MACRO_SELECTOR(OATPP_MACRO_API_CONTROLLER_AUTHORIZATION_, \
                                            TYPE,                                      \
...
#define OATPP_MACRO_API_CONTROLLER_AUTHORIZATION_INFO_1(TYPE, NAME)                                                    \
  ...
  if(__param_obj_##NAME) {                                                                                             \
    ...
    throw oatpp::web::protocol::http::HttpError(                                                                       \
     Status::CODE_500,                                                                                                 \
     "No authorization handler set up in controller before controller was added to router or swagger-doc.");           \
  }
...
#define OATPP_MACRO_API_CONTROLLER_AUTHORIZATION_INFO_2(TYPE, NAME, AUTH_HANDLER)                             \
  ...
  if(__auth_handler_##NAME) {                                                                                 \
    info->headers.add(oatpp::web::protocol::http::Header::AUTHORIZATION, oatpp::String::Class::getType());    \
    info->headers [ oatpp::web::protocol::http::Header::AUTHORIZATION ].description =                         \
     __auth_handler_##NAME->getScheme();                                                                      \
    info->authorization = __auth_handler_##NAME->getScheme();                                                 \
  } else {                                                                                                    \
    throw oatpp::web::protocol::http::HttpError(                                                              \
     Status::CODE_500,                                                                                        \
     "Invalid authorization handler given (or not set up) in AUTHORIZATION(TYPE, NAME, AUTH_HANDLER) before " \
     "controller was added to router or swagger-doc.");                                                       \
  }

#define OATPP_MACRO_API_CONTROLLER_AUTHORIZATION_INFO(TYPE, PARAM_LIST)                     \
  OATPP_MACRO_API_CONTROLLER_MACRO_SELECTOR(OATPP_MACRO_API_CONTROLLER_AUTHORIZATION_INFO_, \
                                            TYPE,                                           \
                                            OATPP_MACRO_UNFOLD_VA_ARGS PARAM_LIST)
