...
/**
 * Macro to declare object pool class which uses &id:oatpp::base::memory::ThreadLocalPoolSharedObjectAllocator; to
 * allocate objects.
 * @param NAME - name of the memory pool.
 * @param TYPE - type of the object.
 * @param CHUNK_SIZE - chunk size for &id:oatpp::base::memory::MemoryPool;.
...
#define OBJECT_POOL(POOL_NAME, TYPE, CHUNK_SIZE)                                                                        \
  ...
  {                                                                                                                     \
    if(sz != sizeof(TYPE)) {                                                                                            \
      oatpp::base::Environment::log(2,                                                                                  \
                                    #POOL_NAME,                                                                         \
                                    "[ERROR|CRITICAL]: MemoryPool malfunction. Deleting object of wrong size");         \
    }                                                                                                                   \
