...
public:
  ...
  String(const char* data1, v_buff_size size1, const char* data2, v_buff_size size2)
    : type::ObjectWrapper<base::StrBuffer, __class::String>(
       base::StrBuffer::createSharedConcatenated(data1, size1, data2, size2))
  {
  }
  ...
  inline bool operator==(const char* str) const
  {
    ...
    if(str == nullptr)
      return false;
    if(m_ptr->getSize() != std::strlen(str))
      return false;
...
public:
  ...
  template<typename T, typename enabled = typename std::enable_if<std::is_same<T, std::nullptr_t>::value, void>::type>
  ...
  template<typename T, typename enabled = typename std::enable_if<std::is_same<T, TValueType>::value, void>::type>
  ...
  template<typename T, typename enabled = typename std::enable_if<std::is_same<T, Primitive>::value, void>::type>
...
public:
  ...
  template<typename T, typename enabled = typename std::enable_if<std::is_same<T, bool>::value, void>::type>
  inline bool operator==(T value) const
  {
    if(!this->m_ptr)
      return false;
