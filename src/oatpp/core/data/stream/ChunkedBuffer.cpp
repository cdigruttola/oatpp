...
const v_buff_size ChunkedBuffer::CHUNK_ENTRY_SIZE = (1 << ChunkedBuffer::CHUNK_ENTRY_SIZE_INDEX_SHIFT);
...
v_io_size ChunkedBuffer::writeToEntry(ChunkEntry* entry, const void* data, v_buff_size count, v_buff_size& outChunkPos)
{
...
v_io_size ChunkedBuffer::writeToEntryFrom(
...
v_io_size ChunkedBuffer::readSubstring(void* buffer, v_buff_size pos, v_buff_size count)
{
...
oatpp::async::CoroutineStarter ChunkedBuffer::flushToStreamAsync(const std::shared_ptr<OutputStream>& stream)
{
  ...
  public:
    FlushCoroutine(const std::shared_ptr<ChunkedBuffer>& chunkedBuffer, const std::shared_ptr<OutputStream>& stream)
