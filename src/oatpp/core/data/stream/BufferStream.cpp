  ...
  if(m_position + count > m_capacity) {
    ...
    if(m_growBytes <= 0) {
      throw std::runtime_error(
       "[oatpp::data::stream::BufferOutputStream::reserveBytesUpfront()]: Error. Buffer was not allowed to grow.");
    }
...
oatpp::async::CoroutineStarter BufferOutputStream::flushToStreamAsync(const std::shared_ptr<BufferOutputStream>& _this,
...
BufferInputStream::BufferInputStream(const std::shared_ptr<base::StrBuffer>& memoryHandle,
                                     p_char8 data,
                                     v_buff_size size)
