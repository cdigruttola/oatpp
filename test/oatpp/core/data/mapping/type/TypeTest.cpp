...
void TypeTest::onRun()
{
  ...
  OATPP_ASSERT(obj->field_list_string.valueType->classId.id ==
               oatpp::data::mapping::type::__class::AbstractList::CLASS_ID.id);

  OATPP_LOGV(TAG, "type: '%s'", obj->field_list_int32.valueType->classId.name);
  OATPP_ASSERT(obj->field_list_int32.valueType->classId.id ==
               oatpp::data::mapping::type::__class::AbstractList::CLASS_ID.id);

  OATPP_LOGV(TAG, "type: '%s'", obj->field_list_int64.valueType->classId.name);
  OATPP_ASSERT(obj->field_list_int64.valueType->classId.id ==
               oatpp::data::mapping::type::__class::AbstractList::CLASS_ID.id);

  OATPP_LOGV(TAG, "type: '%s'", obj->field_list_float32.valueType->classId.name);
  OATPP_ASSERT(obj->field_list_float32.valueType->classId.id ==
               oatpp::data::mapping::type::__class::AbstractList::CLASS_ID.id);

  OATPP_LOGV(TAG, "type: '%s'", obj->field_list_float64.valueType->classId.name);
  OATPP_ASSERT(obj->field_list_float64.valueType->classId.id ==
               oatpp::data::mapping::type::__class::AbstractList::CLASS_ID.id);

  OATPP_LOGV(TAG, "type: '%s'", obj->field_list_boolean.valueType->classId.name);
  OATPP_ASSERT(obj->field_list_boolean.valueType->classId.id ==
               oatpp::data::mapping::type::__class::AbstractList::CLASS_ID.id);

  OATPP_LOGV(TAG, "type: '%s'", obj->field_map_string_string.valueType->classId.name);
  OATPP_ASSERT(obj->field_map_string_string.valueType->classId.id ==
               oatpp::data::mapping::type::__class::AbstractPairList::CLASS_ID.id);
