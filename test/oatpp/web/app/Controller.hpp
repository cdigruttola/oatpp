...
public:
  ...
  ENDPOINT("GET", "params/{param}", getWithParams, PATH(String, param))
  {
  ...
  ENDPOINT("GET", "queries", getWithQueries, QUERY(String, name), QUERY(Int32, age))
  {
    auto dto = TestDto::createShared();
  ...
  ENDPOINT("GET", "queries/map", getWithQueriesMap, QUERIES(QueryParams, queries))
  {
    auto dto = TestDto::createShared();
  ...
  ENDPOINT("GET", "headers", getWithHeaders, HEADER(String, param, "X-TEST-HEADER"))
  {
  ...
  ENDPOINT("POST", "body", postBody, BODY_STRING(String, body))
  {
  ...
  ENDPOINT("POST", "body-dto", postBodyDto, BODY_DTO(Object<TestDto>, body))
  {
    // OATPP_LOGV(TAG, "POST body %s", body->c_str());
  ...
  ENDPOINT("GET", "header-value-set", headerValueSet, HEADER(String, valueSet, "X-VALUE-SET"))
  {

    oatpp::web::protocol::http::HeaderValueData valueData;
    oatpp::web::protocol::http::Parser::parseHeaderValueData(valueData, valueSet, ',');

    OATPP_ASSERT_HTTP(valueData.tokens.find("VALUE_1") != valueData.tokens.end(),
                      Status::CODE_400,
                      "No header 'VALUE_1' in value set");
    OATPP_ASSERT_HTTP(valueData.tokens.find("VALUE_2") != valueData.tokens.end(),
                      Status::CODE_400,
                      "No header 'VALUE_2' in value set");
    OATPP_ASSERT_HTTP(valueData.tokens.find("VALUE_3") != valueData.tokens.end(),
                      Status::CODE_400,
                      "No header 'VALUE_3' in value set");
  ...
  ENDPOINT("POST", "test/multipart-all", multipartFileTest, REQUEST(std::shared_ptr<IncomingRequest>, request))
  {
    ...
    multipartReader.setPartReader("part2",
                                  multipart::createFilePartReader("/Users/leonid/Documents/test/my-text-file.tf"));
  ...
  ENDPOINT("GET", "enum/as-string", testEnumString, HEADER(Enum<AllowedPathParams>::AsString, enumValue, "enum"))
  {
    return createResponse(Status::CODE_200, "OK");
  }

  ENDPOINT("GET", "enum/as-number", testEnumNumber, HEADER(Enum<AllowedPathParams>::AsNumber, enumValue, "enum"))
  {
    return createResponse(Status::CODE_200, "OK");
  }
};
